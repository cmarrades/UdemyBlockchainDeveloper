...
it('lets user1 get the funds after the sale', async() => {
    ...
    const balanceOfSellerBeforeSell = await web3.eth.getBalance(user1);

    await instance.buyStar(starId, {from: user2, value: balance});
    const balanceOfSellerAfterSell = await web3.eth.getBalance(user1);
...
it('lets user2 buy a star, if it is put up for sale', async() => {
    ...
    await instance.approve(user2, starId, {from: user1, gasprice:0}) //this is new added code
    await instance.buyStar(starId, {from: user2, value: balance});
